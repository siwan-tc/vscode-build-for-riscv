...
if [[ "${VSCODE_ARCH}" == "x64" ]]; then
  ...
  # add update's url
  sed -i 's/generate_type2_appimage/generate_type2_appimage -u "gh-releases-zsync|VSCode|vscode|latest|*.AppImage.zsync"/' pkg2appimage.AppDir/AppRun

  # remove check so build in docker can succeed
  sed -i 's/grep docker/# grep docker/' pkg2appimage.AppDir/usr/share/pkg2appimage/functions.sh

  if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
    sed -i 's|@@NAME@@|VSCode-Insiders|g' recipe.yml
    sed -i 's|@@APPNAME@@|code-insiders|g' recipe.yml
    sed -i 's|@@ICON@@|vscode-insiders|g' recipe.yml
  else
    sed -i 's|@@NAME@@|VSCode|g' recipe.yml
    sed -i 's|@@APPNAME@@|code|g' recipe.yml
    sed -i 's|@@ICON@@|vscode|g' recipe.yml
  fi
