...
build_darwin_main() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/darwin/code.icns" ]]; then
    rsvg-convert -w 655 -h 655 "icons/${QUALITY}/code_cnl.svg" -o "code_logo.png"
...
build_darwin_types() { # {{{
  rsvg-convert -w 128 -h 128 "icons/${QUALITY}/code_cnl_w80_b8.svg" -o "code_logo.png"
...
build_linux_main() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/linux/code.png" ]]; then
    wget "https://raw.githubusercontent.com/VSCode/icons/main/icons/linux/circle1/${COLOR}/paulo22s.png" -O "${SRC_PREFIX}src/${QUALITY}/resources/linux/code.png"
  fi
...
build_media() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg" ]]; then
    cp "icons/${QUALITY}/code_clt.svg" "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg"
...
build_windows_main() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/win32/code.ico" ]]; then
    wget "https://raw.githubusercontent.com/VSCode/icons/main/icons/win32/nobg/${COLOR}/paulo22s.ico" -O "${SRC_PREFIX}src/${QUALITY}/resources/win32/code.ico"
  fi
} # }}}
...
build_windows_type() {
  ...
  if [[ ! -f "${FILE_PATH}" ]]; then
    ...
    rsvg-convert -w "${LOGO_SIZE}" -h "${LOGO_SIZE}" "icons/${QUALITY}/code_cnl.svg" -o "code_logo.png"
...
build_windows_types() { # {{{
  ...
  rsvg-convert -b "#F5F6F7" -w 64 -h 64 "icons/${QUALITY}/code_cnl.svg" -o "code_logo.png"
