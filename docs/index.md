...
- [Extensions compatibility](https://github.com/VSCode/vscode/blob/master/docs/extensions-compatibility.md)
- [Migrating from Visual Studio Code to VSCode](#migrating)
- [Sign in with GitHub](#signin-github)
- [Accounts authentication](https://github.com/VSCode/vscode/blob/master/docs/accounts-authentication.md)
- [How do I run VSCode in portable mode?](#portable)
- [How do I fix the default file manager?](#file-manager)
- [How do I press and hold a key and have it repeat in VSCode?](#press-and-hold)
- [How do I open VSCode from the terminal?](#terminal-support)
  - [From Linux .tar.gz](#from-linux-targz)
- [Troubleshooting](https://github.com/VSCode/vscode/blob/master/docs/troubleshooting.md)
- [Contributing](https://github.com/VSCode/vscode/blob/master/CONTRIBUTING.md)
- [How to build VSCode](https://github.com/VSCode/vscode/blob/master/docs/howto-build.md)

## <a id="disable-telemetry"></a>Getting all the Telemetry Out

Even though we do not pass the telemetry build flags (and go out of our way to cripple the baked-in telemetry), Microsoft will still track usage by default.

We do however set the default `telemetry.enableCrashReporter` and `telemetry.enableTelemetry` values to false. You can see those by viewing your VSCode settings.json and searching for `telemetry`.
...
When searching the `@tag:usesOnlineServices` filter, note that while the "Update: Mode" setting description still says "The updates are fetched from a Microsoft online service", VSCode's build script [sets the `updateUrl` field](https://github.com/VSCode/vscode/blob/master/prepare_vscode.sh#L36) in `product.json` to that of VSCode's own small [update server](https://github.com/VSCode/update-api), so enabling that setting won't actually result in any calls to Microsoft servers.

Likewise, while the descriptions for "Extensions: Auto Check Updates" and "Extensions: Auto Update" include the same phrase, VSCode [replaces](https://github.com/VSCode/vscode/blob/master/prepare_vscode.sh#L42) the Visual Studio Marketplace with Open VSX, so these settings won't call Microsoft, either.

## <a id="extensions-marketplace"></a>Extensions + Marketplace

Being a vscode based editor, VSCode gets additional features by installing VS Code extensions.
...
As noted above, the [Open VSX Registry](https://open-vsx.org/) is the pre-set extension gallery in VSCode. Using the extension view in VSCode will therefore by default use it.
...
Or by creating a custom `product.json` at the following location (replace `VSCode` by `VSCode - Insiders` if you use that):
- Windows: `%APPDATA%\VSCode` or `%USERPROFILE%\AppData\Roaming\VSCode`
- macOS: `~/Library/Application Support/VSCode`
- Linux: `$XDG_CONFIG_HOME/VSCode` or `~/.config/VSCode`
...
* [Open VSX](https://github.com/eclipse/openvsx) eclipse open-source project
  While the public instance which is run by the Eclipse Foundation is the pre-set endpoint in VSCode, you can host your own instance.
...
A workaround exists to get debugging working in C# projects, by using Samsung's opensource [netcoredbg](https://github.com/Samsung/netcoredbg) package. See [this comment](https://github.com/VSCode/vscode/issues/82#issue-409806641) for instructions on how to set that up.

### <a id="proprietary-extensions"></a>Proprietary Extensions

Like the debuggers mentioned above, some extensions you may find in the marketplace (like the [Remote Development Extensions](https://code.visualstudio.com/docs/remote/remote-overview)) only function with the official Visual Studio Code build. You can work around this by adding the extension's internal ID (found on the extension's page) to the `extensionAllowedProposedApi` property of the product.json in your VSCode installation. For example:
...
## <a id="migrating"></a>Migrating from Visual Studio Code to VSCode

VSCode (and a freshly cloned copy of vscode built from source) stores its extension files in `~/.vscode-oss`. So if you currently have Visual Studio Code installed, your extensions won't automatically populate. You can reinstall your extensions from the Marketplace in VSCode, or copy the `extensions` from `~/.vscode/extensions` to `~/.vscode-oss/extensions`.
...
You can copy these files to the VSCode user settings folder:

- __Windows__: `%APPDATA%\VSCode\User`
- __macOS__: `$HOME/Library/Application Support/VSCode/User`
- __Linux__: `$HOME/.config/VSCode/User`

To copy your settings manually:

- In Visual Studio Code, go to Settings (Command+, if on a Mac)
- Click the three dots `...` and choose 'Open settings.json'
- Copy the contents of settings.json into the same place in VSCode

## <a id="signin-github"></a>Sign in with GitHub

In VSCode, `Sign in with GitHub` is using a Personal Access Token.<br />
...
## <a id="portable"></a>How do I run VSCode in portable mode?
...
- **macOS** : portable mode is enabled by the existence of a specially named folder. For Visual Studio Code that folder name is `code-portable-data`. For VSCode, that folder name is `code-portable-data`. So to enable portable mode for VSCode on Mac OS, follow the instructions outlined in the [link above](https://code.visualstudio.com/docs/editor/portable), but create a folder named `code-portable-data` instead of `code-portable-data`.

## <a id="file-manager"></a>How do I fix the default file manager (Linux)?

In some cases, VSCode becomes the file manager used to open directories (instead of apps like Dolphin or Nautilus).<br />
...
## <a id="press-and-hold"></a>How do I press and hold a key and have it repeat in VSCode (Mac)?

This is a common question for Visual Studio Code and the procedure is slightly different in VSCode because the `defaults` path is different.

```bash
$ defaults write com.vscode ApplePressAndHoldEnabled -bool false
```

## <a id="terminal-support"></a>How do I open VSCode from the terminal?
...
This allows you to open files or directories in VSCode directly from your terminal:

```bash
~/in-my-project $ code . # open this directory
~/in-my-project $ code file.txt # open this file
```

Feel free to alias this command to something easier to type in your shell profile (e.g. `alias code=code`).

On Linux, when installed with a package manager, `code` has been installed in your `PATH`.

### <a id="from-linux-targz"></a>From Linux .tar.gz

When the archive `VSCode-linux-<arch>-<version>.tar.gz` is extracted, the main entry point for VSCode is `./bin/code`.
