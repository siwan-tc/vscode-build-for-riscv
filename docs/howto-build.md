...
To build for Linux, you can alternatively build VSCode in docker

### <a id="build-docker-x64"></a>X64

Firstly, create the container with:
```
docker run -ti --volume=<local vscode source>:/root/vscode --name=vscode-build-agent vscode/vscode-linux-build-agent:bionic-x64 bash
```
like
```
docker run -ti --volume=$(pwd):/root/vscode --name=vscode-build-agent vscode/vscode-linux-build-agent:bionic-x64 bash
```
...
Firstly, create the container with:
```
docker run -ti --volume=<local vscode source>:/root/vscode --name=vscode-build-agent vscode/vscode-linux-build-agent:bionic-armhf bash
```
like
```
docker run -ti --volume=$(pwd):/root/vscode --name=vscode-build-agent vscode/vscode-linux-build-agent:bionic-armhf bash
```
...
# verify the snap
review-tools.snap-review --allow-classic code*.snap
```
...
- run `./build/build_<os>.sh`, if a patch is failing then,
- open `vscode` directory in **VSCode**
- revert all changes
